...
class Schemas {
  ...
  add(schema) {
    if (this.validateSchema(schema)) {
      ...
      NAF.log.error(
        'See https://github.com/haydenjameslee/networked-aframe#syncing-custom-components'
      );
    }
  }
  ...
  validateTemplate(schema, el) {
    if (!this.isTemplateTag(el)) {
      NAF.log.error(
        `Template for ${schema.template} is not a <template> tag. Instead found: ${el.tagName}`
      );
      return false;
...
class WebrtcAdapter {
  ...
  connect() {
    ...
    this.updateTimeOffset().then(() => {
      ...
      NAF.log.write('Attempting to connect to socket.io');
      const socket = (self.socket = io(self.wsUrl, {
        withCredentials: false, // これ必要そう。cors関連
      }));
...
function getNetworkedEntity(entity) {
  return new Promise((resolve, reject) => {
    ...
    while (
      curEntity &&
      curEntity.components &&
      !curEntity.components.networked
    ) {
